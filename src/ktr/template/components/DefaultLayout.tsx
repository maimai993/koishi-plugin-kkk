      ...
      {version ? (
        <div className="relative z-50 pt-32 pb-20 text-foreground/80">
          ...
          <div className="flex relative justify-center items-center space-x-8">
            ...
            <div className="flex items-end space-x-8">
              {/*
                上游这里是 src="/image/frame-logo.png"（绝对路径，靠 karin 的 HTTP 服务提供）。
                Koishi 移植版渲染的是本地 HTML 文件，绝对路径会变成裂图，
                所以改用渲染上下文里的 data URI（见 Render 的 resolveFrameLogo）。
              */}
              {(version as any).frameLogo ? (
                <img src={(version as any).frameLogo} alt="logo" className="w-auto h-18" />
              ) : null}
            ...
            {/* 作者署名：by + 头像 + 名字（头像地址是外链，渲染时会被浏览器直接拉取） */}
            <div className="flex items-center space-x-3">
              <div className="w-1 h-14 opacity-90 bg-foreground/70" />
              <img
                src="https://q1.qlogo.cn/g?b=qq&nk=2237886846&s=640"
                alt="maimai"
                className="w-16 h-16 rounded-full object-cover"
                referrerPolicy="no-referrer"
              />
              <div className="flex flex-col items-start">
                <span className="text-sm font-bold tracking-widest uppercase opacity-80">By</span>
                <span className="text-3xl font-black leading-none opacity-90">maimai</span>
              </div>
            </div>

            {/* 构建工具信息整块去掉：ROLLDOWN / VITE 都不要了 */}
          </div>
        </div>
      ) : (
        <div className="h-24" />
