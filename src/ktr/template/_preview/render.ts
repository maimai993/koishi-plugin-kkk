...
const findCoreRoot = (): string => {
  // Koishi 移植：产物是 CJS，没有 import.meta；直接取 __dirname。
  // 同时把包名判断放宽到移植后的包名（koishi-plugin-kkk），否则永远找不到包根。
  ...
  for (let i = 0; i < 10; i++) {
    ...
    if (fs.existsSync(pkgPath)) {
      try {
        ...
        if (name === 'koishi-plugin-kkk' || name === 'koishi-plugin-kkk') {
          return dir
        }
      } catch {
