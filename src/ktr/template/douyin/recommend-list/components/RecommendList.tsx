...
import type { DouyinRecommendListData } from './types'

/**
 * 抖音推荐列表组件
 */
export const DouyinRecommendList: React.FC<PosterProps<DouyinRecommendListData>> = (props) => {
          ...
          <div className="flex flex-col gap-10 relative">
            ...
            <div className="flex flex-col items-center text-center">
              <span className="text-6xl font-black text-foreground leading-tight relative">
                {props.data.recommender_username}
                {/* 装饰图形 */}
                <svg className="absolute -right-8 -top-2 text-blue-400 w-8 h-8 opacity-60" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="col-span-8 h-full">
          <div className="h-full bg-surface/60 backdrop-blur-xl rounded-[3rem] p-10 border border-border flex flex-col gap-8 shadow-2xl relative overflow-hidden">
            ...
            <div className="grid grid-cols-5 gap-6 z-10">
              <StatItem icon={DouyinLikeIcon} value={props.data.dianzan} />
              <StatItem icon={DouyinRecommendIcon} value={props.data.tuijian} iconColor="#13C15A" />
