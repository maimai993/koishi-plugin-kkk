    ...
    <div className="overflow-hidden relative rounded-6xl">
      <div className="flex relative z-10 gap-25 p-25">
        <div className="flex flex-col shrink-0 gap-20">
          ...
          {/** TIP：只有「卡片里画了分集列表」时才提示文字选集；QQ 面板改用表格按钮，这里就不显示 */}
          {(props as any).showEpisodeTip !== false && (
    ...
    <DefaultLayout {...props}>
      <div className="p-4">
        ...
        <BangumiBilibiliHeader
          // 卡片里有没有画分集列表：有才提示「发送第？集」（QQ 面板用表格按钮，这时不提示）
          showEpisodeTip={Array.isArray(props.data.Episodes) && props.data.Episodes.length > 0}
        ...
        {/* 剧集列表区域：QQ 面板会把分集改成表格按钮，这时传空数组就不渲染（否则卡片会拉成超长图） */}
        {props.data.Episodes && props.data.Episodes.length > 0 && <BangumiBilibiliEpisodes {...props.data} />}
      </div>
    </DefaultLayout>
  )
})
