...
已经注册成真正的 Koishi 指令（**不带 \`#\` 前缀**，前缀按 Koishi 的配置来），例如
\`解析\` / \`弹幕解析\` / \`kkk帮助\` / \`kkk版本\` / \`kkk解析统计\` / \`B站登录\` / \`设置抖音推送\` …
发送**裸链接**（或回复一条含链接的消息）也会自动解析，由 \`autoParse\` 控制。
旧的 \`#解析\` 这种写法仍可用（兼容保留）。
...
代价：在表单里把某项**改回默认值**不会写回文件 —— 真要改回去请直接编辑那个文件。

想做结构化编辑（例如清空推送列表）时，直接编辑数据目录下的
\`<dataPath>/koishi-plugin-kkk/config/config.json\`，重启后生效。
`
