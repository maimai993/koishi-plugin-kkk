...
const PLAYER_SCRIPT = [
  ...
  "  /**",
  "   * 弹幕洪峰时的取舍（互动稿的切片经常是「6 秒里 500 条」）。",
  "   *",
  "   * 以前的做法是「轨道满了就丢」，于是白字滚动弹幕（占九成）把四条轨道一直占死，",
  "   * 彩色弹幕和顶部弹幕（本来就少）**永远轮不到** —— 用户实测反馈就是这个：",
  "   * 「彩色弹幕一个都没有、顶部弹幕也不显示」。",
  "   * 现在：白字滚动的按每秒预算抽稀，彩色 / 顶部 / 底部**不吃预算**照常放行。",
  "   */",
  "  var dmBudget = 0",
  "  var DM_BUDGET_PER_SEC = 18",
  "",
  ...
  "    var isRare = kind !== 'scroll' || Number(item.color) !== 0xffffff",
  "    // 白字滚动走预算（洪峰时抽稀）；彩色 / 顶 / 底本来就不多，照常放行",
  "    if (!isRare) {",
  "      if (dmBudget < 1) return",
  "      dmBudget -= 1",
  "    }",
  ...
  "    // 滚动弹幕按显示区域限量（多了糊成一片）；顶/底宁可叠在一起也不丢",
  "    if (kind === 'scroll' ? lane >= laneCount() : lane >= laneCount() * 3) return",
  "    active.push({",
  ...
  "    if (dt > 0) dmBudget = Math.min(DM_BUDGET_PER_SEC, dmBudget + dt * DM_BUDGET_PER_SEC)",
  "    var timeMs = video.currentTime * 1000",
  "    var guard = 0",
  ...
  "    audioTrack.addEventListener('error', function () {",
  "      /**",
  "       * 音轨失败要给得出细节：光一句「声音加载失败」在排查时等于没说",
  "       * （code 1=自己中止 2=网络 3=解码 4=格式不支持）。",
  "       */",
  "      var mediaError = audioTrack.error || {}",
  "      console.warn('[kkk] 音轨加载失败', { code: mediaError.code, message: mediaError.message, networkState: audioTrack.networkState, readyState: audioTrack.readyState, src: audioTrack.currentSrc || audioTrack.src })",
  "      showNotice('声音加载失败（代码 ' + (mediaError.code || '?') + '），画面继续播放；可以在下面单独下载声音')",
  "    })",
  "  }",
  ...
  "    // 互动稿每一段的弹幕池是各是各的：换段就得换弹幕，不然飘的还是上一段那批",
  "    loadDanmaku(choice.cid)",
  ...
  "",
  "  /**",
  "   * 换弹幕：`/danmaku?cid=<这一段>`（互动稿每段一个弹幕池）。",
  "   *",
  "   * 取到就直接替换画面上的弹幕（光标、轨道一起清），取不到就保留原来的 —— 弹幕出问题不该影响看视频。",
  "   */",
  "  function loadDanmaku (cid) {",
  "    fetch(API + '/danmaku' + (cid ? '?cid=' + cid : ''))",
  "      .then(function (res) { return res.ok ? res.json() : null })",
  "      .then(function (data) {",
  ...
  "        reset(0)",
  "      })",
  "      .catch(function () { /* 保留原来的弹幕，别因为接口抽风把画面清空 */ })",
  "  }",
  "",
  ...
  "    console.warn('[kkk] 分段画面加载失败', { code: videoError.code, message: videoError.message, src: video.currentSrc || video.src, reason: storyLastReason })",
  "    storyPending = null",
  "    if (storyPollTimer) { window.clearTimeout(storyPollTimer); storyPollTimer = 0 }",
  "    if (notice) notice.hidden = true",
