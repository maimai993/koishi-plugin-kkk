...
const PLAYER_SCRIPT = [
  ...
  "",
  "  /**",
  "   * 点了一条选项：**先让服务端把这一段下好，再换源**。",
  "   *",
  "   * 以前是直接改 video.src —— 服务端要现取流、现下载、现合成，好几秒里用户面对的是一块黑屏。",
  "   * 现在：HEAD 触发下载（不带 body）→ 轮询 /progress 显示百分比与阶段 → ready 之后再换源",
  "   * （那时候已经是会话目录里的本地文件，换完秒开）。",
  "   */",
  ...
  "  }",
  "",
  "  /** 轮询分段准备进度：ready → 换源播放；failed → 明确说一句 */",
  "  function pollProgress (cid, waited) {",
  "    if (storyPending !== cid) return",
  "    if (waited > 600) {",
  "      storyPending = null",
  "      renderProgress('这一段准备超时了，刷新页面重来一次', 0)",
  "      return",
  "    }",
  ...
  "      .then(function (info) {",
  "        if (storyPending !== cid) return",
  "        if (info && info.stage === 'ready') { switchSegment(cid); return }",
  "        if (info && info.stage === 'failed') {",
  "          storyPending = null",
  "          renderProgress('这一段没准备好（下载失败或链接失效），刷新页面重来一次', 0)",
  "          return",
  "        }",
  "        var stage = info ? info.stage : 'idle'",
  "        var percent = info && info.percent ? info.percent : 0",
  "        var text = stage === 'merging' ? '正在合成这一段…'",
  "          : stage === 'audio' ? '正在下载声音 ' + percent + '%'",
  "            : stage === 'queued' ? '排队准备中…'",
  "              : '正在下载这一段 ' + percent + '%'",
  "        renderProgress(text, stage === 'merging' ? 100 : percent)",
  ...
  "",
  "  /**",
  "   * 分段下好了：换 src 接着播（这时候已经是本地文件了）。",
  "   *",
  "   * 画面和声音是**两条流**（B站就这样，我们也不在服务器合成）：",
  "   * 一起换成同一段的 '\/segment/<cid>' 与 '\/segment/<cid>?audio=1'，",
  "   * 再按原来那套对齐逻辑（play → playAudio → alignAudio）把两者拉齐。",
  "   */",
  "  function switchSegment (cid) {",
  "    if (storyPollTimer) { window.clearTimeout(storyPollTimer); storyPollTimer = 0 }",
  "    storyPending = null",
  "    hideStory()",
  "    if (notice) notice.hidden = true",
  "    video.setAttribute('src', API + '/segment/' + cid)",
  "    if (audioTrack) {",
  "      audioTrack.setAttribute('src', API + '/segment/' + cid + '?audio=1')",
  ...
  "  }",
  "",
  "  /** 快放完了（剩 6 秒）或已经放完：把选项贴上来（放完时停在结束画面上，不会跳走） */",
  "  function maybeShowStory () {",
  "    if (!storyNode || storyPending || storyShown) return",
  "    var left = video.duration - video.currentTime",
  "    if (video.ended || (isFinite(left) && left <= 6)) {",
  "      renderStory()",
  "      wakeControls()",
  "    }",
  "  }",
  "  video.addEventListener('timeupdate', maybeShowStory)",
  "  video.addEventListener('ended', maybeShowStory)",
  "",
  "  /** 分段加载失败：给一句对得上的提示（页面自带的 error 文案说的是「链接过期」） */",
  "  video.addEventListener('error', function () {",
