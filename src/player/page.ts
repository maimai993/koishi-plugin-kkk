...
const PLAYER_STYLE = [
  ...
  /* 互动视频的选项覆盖层：浮在画面上，全屏时（.video-wrap 整个进全屏）跟着一起进去 */
  '.story{position:absolute;left:0;right:0;top:0;bottom:0;z-index:5;display:flex;flex-direction:column;',
  'justify-content:flex-end;gap:10px;padding:16px;line-height:1.5;color:#fff;',
  'background:linear-gradient(180deg,rgba(0,0,0,.18),rgba(0,0,0,.84));}',
  '.story[hidden]{display:none}',
  '.story-q{font-size:15px;font-weight:600;text-shadow:0 1px 3px rgba(0,0,0,.6);}',
  '.story-tip{font-size:12px;color:#c9ccd3}',
  '.story-opts{display:flex;flex-direction:column;gap:8px;max-height:62%;overflow:auto;}',
  '.story-opt{appearance:none;display:flex;align-items:center;gap:10px;width:100%;padding:10px 12px;text-align:left;',
  'border:1px solid rgba(255,255,255,.22);border-radius:10px;background:rgba(22,23,26,.86);color:#fff;',
...
const PLAYER_SCRIPT = [
  ...
  "  }",
  "",
  "  /** 把当前这一段的题目与选项画出来（纯 DOM 拼，用户文字一律 textContent，别引 XSS） */",
  "  function renderStory () {",
  "    if (!storyEl || !storyNode) return",
  "    storyEl.innerHTML = ''",
  "    var head = document.createElement('div')",
  "    head.className = 'story-q'",
  "    // 文案里不能有 emoji：播放页是纯 SVG 图标 + 文字（smoke-player 有这条断言）",
  "    head.textContent = storyNode.isLeaf ? '互动视频走到结局了' : (storyNode.question || '请选择接下来的剧情')",
  ...
  "      tip.textContent = '点一条接着看（下一段是按需下载的，第一次要等几秒）'",
  "      storyEl.appendChild(tip)",
  "    } else {",
  "      var ending = document.createElement('div')",
  "      ending.className = 'story-tip'",
  "      ending.textContent = '这一段没有后续了，可以重看，或者关掉页面'",
  "      storyEl.appendChild(ending)",
  "    }",
  "    storyEl.hidden = false",
  "  }",
  "",
  "  /** 换到某一段：视频地址指向 /segment/<cid>，服务端没这份文件时会现下（下完留在会话目录里） */",
  ...
  "    wakeControls()",
  "  })",
  "  // 下一段真的开始播了 → 收起「正在准备」提示",
  "  video.addEventListener('playing', function () {",
  "    if (!storyPending) return",
  "    storyPending = false",
  "    if (notice) notice.hidden = true",
  "  })",
  "  // 分段下载失败：给一句对得上的提示（页面自带的 error 文案说的是「链接过期」，对不上）",
  "  video.addEventListener('error', function () {",
  "    if (!storyPending) return",
  "    storyPending = false",
  "    showNotice('这一段的视频没准备好（下载失败或链接失效）。刷新页面可以从头再来一次')",
  "  })",
  "  // 点面板背景 = 先不看选项（点按钮不会冒泡到这里，按钮里已经 stopPropagation）",
  "  if (storyEl) storyEl.addEventListener('click', function (event) { if (event.target === storyEl) hideStory() })",
  "  // 控制条上的「选项」按钮：走到一半想提前看选项时点它（全屏时也在控制条上）",
  "  if (storyBtn) {",
