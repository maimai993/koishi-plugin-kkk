 ...
 *   3. 用浏览器渲染服务（koishi-plugin-puppeteer / puppeteer-without-canvas）截图；3.5.0 起不再支持 shotkit 内核。
 ...
 * 3.5.0 起不再支持 shotkit 内核：它的预编译内核在 Windows 上加载不了 https 资源
 * （卡片里的封面、头像全是空白框），换来的那点速度不值得让卡片看起来是坏的。
 *
 * scale 会作为 deviceScaleFactor（配置项 app.renderScale，100 = 1x，200 = 2x）：
 * 不设的话高分辨率卡片会被截成 1x，群里看着又小又糊。
 */
async function screenshot (htmlPath: string, selector: string, timeout: number, scale = 1, format: 'png' | 'jpeg' = 'jpeg'): Promise<{ base64: string; mime: string; dsf: number }> {
  ...
  if (!puppeteer) {
    throw new Error('没有可用的渲染器：需要安装浏览器渲染服务（koishi-plugin-puppeteer 或同类插件）才能渲染图片')
  }
...
export const Render = async (_event: any, route: string, data: any): Promise<any[]> => {
  ...
  try {
    ...
    // mime 由 screenshot 返回实际产出的格式（就是上面选的那个 format）
    const htmlBuiltAt = Date.now()
    const { base64, mime, dsf } = await screenshot(htmlPath, '#container', Number(Config.app?.RenderWaitTime ?? 10) * 1000, scale, format)
