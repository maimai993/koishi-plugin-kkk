...
/**
 * 「伪造合并转发消息」的新说明（用户要求）：
 * 这个开关现在的作用是「解析结果合并转发」——把所有解析产物并成一条转发，
 * 开关本身只决定展示身份（触发者 / 机器人）。
 */
const FAKE_FORWARD_DESC = '一次解析产生的所有内容（卡片、图片、视频、评论…）合并成一条转发消息发出，'
  + '过程提示（开始解析 / 下载中 / 发送中…）不会进转发；只有支持合并转发的适配器有效果，'
  + 'QQ 官方适配器会自动退化成逐条发送。开启后合并转发使用触发者身份展示；关闭后使用机器人身份展示。'

const DESC_BY_LABEL = [
  ['谁可以触发扫码登录', PERM_DESC],
  ['错误日志', LOG_DESC],
  ['伪造合并转发消息', FAKE_FORWARD_DESC],
]
